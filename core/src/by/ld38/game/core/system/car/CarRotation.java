...
public class CarRotation extends IteratingSystem {
    public CarRotation() {
        super(Aspect.all(Car.class, Force.class, Velocity.class , Position.class, Collides.class));}
        @Override
        protected void process(int entityId) {
            ...
            Collides col = world.getMapper(Collides.class).get(entityId);
            if (col.framesColliding>0){
                col.framesColliding--;
                return;}
            if (vel.x == 0 && vel.y == 0) {
                pos.angle = 0;
